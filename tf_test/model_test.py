...
from seq2seqModel import attention_model


from seq2seqModel import basic_model
...
class ModelTest(tf.test.TestCase):
  ...
  def testNoAttentionNoResidualUniEncoder(self):
    ...
    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        train_m = self._createTestTrainModel(basic_model.encoder_decoder_basic, hparams, sess)
    ...
    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        eval_m = self._createTestEvalModel(basic_model.encoder_decoder_basic, hparams, sess)
        self._assertEvalLossAndPredictCount(eval_m, sess,
                                            'NoAttentionNoResidualUniEncoder')

    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        infer_m = self._createTestInferModel(basic_model.encoder_decoder_basic, hparams, sess)
  ...
  def testAttentionMechanismLuong(self):
    ...
    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        train_m = self._createTestTrainModel(attention_model.encoder_decoder_attention,
                                             hparams, sess)
    ...
    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        eval_m = self._createTestEvalModel(attention_model.encoder_decoder_attention,
                                           hparams, sess)
        self._assertEvalLossAndPredictCount(eval_m, sess,
    ...
    with tf.Graph().as_default():
      with tf.Session(worker.target, config=self._get_session_config()) as sess:
        infer_m = self._createTestInferModel(attention_model.encoder_decoder_attention,
                                             hparams, sess)
  ...
  def testBeamSearchBasicModel(self):
    ...
    with self.test_session() as sess:
      infer_m = self._createTestInferModel(
          basic_model.encoder_decoder_basic, hparams, sess, True)
  ...
  def testBeamSearchAttentionModel(self):
    ...
    with self.test_session() as sess:
      infer_m = self._createTestInferModel(
          attention_model.encoder_decoder_attention, hparams, sess, True)
      self._assertBeamSearchOutputs(
