...
def run_sample_decode(infer_model, infer_sess, model_dir, hparams,
  ...
  with infer_model.graph.as_default():
    loaded_infer_model, global_step = modelBuilder.create_or_load_model(
...
def run_internal_eval(
  ...
  with eval_model.graph.as_default():
    loaded_eval_model, global_step = modelBuilder.create_or_load_model(
...
def run_avg_external_eval(infer_model, infer_sess, model_dir, hparams,
  ...
  if hparams.avg_ckpts:
    ...
    avg_model_dir = modelBuilder.avg_checkpoints(
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess,
  ...
  metrics = {
      "dev_ppl_internal_eval": dev_ppl,
      "test_ppl_internal_eval": test_ppl,
      "dev_scores_external_eval": dev_scores,
      "test_scores_external_eval": test_scores,
  }
  ...
  if avg_ckpts:
    avg_dev_scores, avg_test_scores = run_avg_external_eval(
        infer_model, infer_sess, model_dir, hparams, summary_writer,
        global_step)
    metrics["avg_validation_scores"] = avg_dev_scores
...
def print_step_info(prefix, global_step, info, result_summary, log_f):
  utils.print_out(
      ...
      (prefix, global_step, info["learning_rate"], info["avg_step_time"],
       ...
       time.ctime()),
      log_f) # write all info at the current global step
...
def train(hparams, scope=None, target_session=""):
  ...
  utils.print_out("# log_file=%s" % log_file, log_f)# save to log file
  ...
  with train_model.graph.as_default():
    loaded_train_model, global_step = modelBuilder.create_or_load_model(
        train_model.model, model_dir, train_sess, "train")


  summary_writer = tf.summary.FileWriter(
      os.path.join(out_dir, summary_name), train_model.graph)  # summary writer
  ...
  run_full_eval(
      ...
      sample_tgt_data, avg_ckpts)  # First evaluation
  ...
  while global_step < num_train_steps:
    ...
    # statistics should be saved now
    ...
    if global_step - last_eval_step >= steps_per_eval:
      ...
      loaded_train_model.saver.save(
          train_sess,
          os.path.join(out_dir, "attr2capt.ckpt"),
...
def _internal_eval(model, global_step, sess, iterator, iterator_feed_dict,
  ...
  ppl = modelBuilder.compute_perplexity(model, sess, label)
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
  ...
  decode_outputs, attention_summary = model.decode(sess)

  if hparams.beam_width > 0:
    # get the top translation.
    decode_outputs = decode_outputs[0]
  # decode a sentence.
  ...
  utils.print_out("    attributes: %s" % src_data[decode_id])
  utils.print_out("    reference: %s" % tgt_data[decode_id])
  utils.print_out(b"    captions: " + s)

  if attention_summary is not None:
    summary_writer.add_summary(attention_summary, global_step)

# bleu and rough evaluation , default bleu
def _external_eval(model, global_step, sess, hparams, iterator,
  ...
  scores = decode_seq.decode_and_evaluate(
      label,
      model,
      sess,
      output,
      ...
      tgt_eos=hparams.eos,
      decode=decode)
  # save on best metrics in best_bleu
  if decode:
    for metric in hparams.metrics:
      if avg_ckpts:
